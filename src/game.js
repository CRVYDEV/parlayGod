...
import { fighterOf } from './boxing.js';
...
export async function loadOwned(client, ch) {
  ...
  const fighter = await fighterOf(client, ch.id); // the fight-circuit contender, if this man manages one
...
async function persistCharacter(client, ch) {
  await client.query(
    `UPDATE characters SET respect=$2, energy=$3, nerve=$4, health=$5, cash=$6, bank=$7,
      ...
      rwa_used=$57, rwa_at=$58, envelope_until=$59, wire_until=$60 WHERE id=$1`,
...
export function view(ch, acct = {}, owned = {}) {
  ...
  return { id: ch.id, name: ch.name, generation: ch.generation, level: lvl,
    ...
    rackets: owned.rackets || [], assets, businesses: owned.businesses || [], speakeasy: owned.speakeasy || null, fighter: owned.fighter || null, cargo: owned.cargo || {}, items: owned.items || {}, gear,
