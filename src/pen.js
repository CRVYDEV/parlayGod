...
const insideOnly = (ch) => { if (!jailed(ch)) throw new GameError('free', "You're on the outside — the Pen is closed to you."); };
...
export async function penBoard(ch, client, h) {
  ...
  return {
    ...
    protectionCost: PEN.PROTECTION_COST, bribePerSecond: PEN.BRIBE_PER_S,
    yard: roster,
  };
}
...
export async function payProtection(ch, client, h) {
  insideOnly(ch);
  if (Number(ch.cash) < PEN.PROTECTION_COST) throw new GameError('cash', `The yard boss wants $${PEN.PROTECTION_COST}.`);
  ch.cash = Number(ch.cash) - PEN.PROTECTION_COST;
  const base = penSafe(ch) ? new Date(ch.pen_safe_until).getTime() : Date.now();
  ch.pen_safe_until = new Date(base + PEN.PROTECTION_MS);
  await client.query('UPDATE street_tax SET pool = pool + $1 WHERE id=1', [PEN.PROTECTION_COST]);
  await h.ledger(client, { characterId: ch.id, currency: 'cash', amount: -PEN.PROTECTION_COST, reason: 'pen:protection' });
  return { ok: true, protectedSeconds: Math.ceil((new Date(ch.pen_safe_until) - Date.now()) / 1000) };
}
...
export async function shank(ch, victim, client, h) {
  ...
  const p = process.env.SHANK_P != null ? Number(process.env.SHANK_P)
    : Math.max(PEN.SHANK_MIN, Math.min(PEN.SHANK_MAX, PEN.SHANK_BASE + (km - vm) / PEN.SHANK_SCALE));
  const roll = Math.random();
  await h.rngLog(client, ch.id, `shank:${victim.id}`, roll, roll < p ? 'landed' : 'missed');

  if (roll >= p) {
    // caught fumbling — the shiv's gone, the killer eats damage and more time; the mark walks away
    ...
    return { ok: true, kill: false, caught: true, dmg, sentenceSeconds: jailSecondsLeft(ch) };
  }
