...
| | |
|---|---|
| Backend modules | **151** files, **50522** lines (`src/`, incl. `src/routes/` and `src/social/`) |
| Test suites | **109** files, **36083** lines (`test/`) — ratio 0.71 test:src |
| HTTP routes | **688** registrations |
| Database tables | **238** (`schema.sql`, 3532 lines) |
| Client | **8709** lines (`public/index.html`, single file, zero dependencies) |
| Ops dashboard + wiki | `public/admin.html`, `public/wiki.html` |
| Smart contracts | **18** contracts, **3641** lines Solidity, **293** Foundry tests passing |
...
| Design + audit docs | **196** markdown files, **54654** lines — indexed in `docs/AUDITS.md`, which states they are point-in-time |
...
catalog, ladder and founder-signed lever (5606 lines) and the extractor never opens it. `rules.js`
...
196 markdown files, 54k lines, with CLAUDE.md alone 13038 lines of dense prose. Two codices already
