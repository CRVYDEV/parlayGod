...
| | |
|---|---|
| Backend modules | **156** files, **53643** lines (`src/`, incl. `src/routes/` and `src/social/`) |
| Test suites | **112** files, **43731** lines (`test/`) — ratio 0.82 test:src |
| HTTP routes | **704** registrations |
| Database tables | **241** (`schema.sql`, 3621 lines) |
| Client | **10509** lines (`public/index.html`, single file, zero dependencies) |
...
| Design + audit docs | **209** markdown files, **61553** lines — indexed in `docs/AUDITS.md`, which states they are point-in-time |
...
catalog, ladder and founder-signed lever (5726 lines) and the extractor never opens it. `rules.js`
