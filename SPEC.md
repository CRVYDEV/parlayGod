...
| | |
|---|---|
| Backend modules | **149** files, **49567** lines (`src/`, incl. `src/routes/` and `src/social/`) |
| Test suites | **106** files, **35137** lines (`test/`) — ratio 0.71 test:src |
| HTTP routes | **688** registrations |
| Database tables | **233** (`schema.sql`, 3473 lines) |
...
| Design + audit docs | **195** markdown files, **54654** lines — indexed in `docs/AUDITS.md`, which states they are point-in-time |
...
catalog, ladder and founder-signed lever (5491 lines) and the extractor never opens it. `rules.js`
