...
| | |
|---|---|
| Backend modules | **117** files, **39102** lines (`src/`, incl. `src/routes/` and `src/social/`) |
| Test suites | **67** files, **26327** lines (`test/`) — ratio 0.67 test:src |
| HTTP routes | **557** registrations |
| Database tables | **188** (`schema.sql`, 2679 lines) |
| Client | **7,156** lines (`public/index.html`, single file, zero dependencies) |
...
| Design + audit docs | **163** markdown files, **41620** lines — indexed in `docs/AUDITS.md`, which states they are point-in-time |
...
catalog, ladder and founder-signed lever (4676 lines) and the extractor never opens it. `rules.js`
