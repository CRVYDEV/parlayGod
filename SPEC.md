...
| | |
|---|---|
| Backend modules | **106** files, **33006** lines (`src/`, incl. `src/routes/` and `src/social/`) |
| Test suites | **60** files, **20232** lines (`test/`) — ratio 0.61 test:src |
| HTTP routes | **514** registrations |
| Database tables | **174** (`schema.sql`, 2381 lines) |
| Client | **6,423** lines (`public/index.html`, single file, zero dependencies) |
...
| Design + audit docs | **138** markdown files, **31245** lines — indexed in `docs/AUDITS.md`, which states they are point-in-time |
...
catalog, ladder and founder-signed lever (3630 lines) and the extractor never opens it. `rules.js`
