...
2. NOTHING MINTS VALUE: VoucherClaim transfers pre-funded OMR only; GearVault mints only via VoucherClaim; Staking pays only from its funded pool. Preserve these invariants and their fuzz tests.
...
5. Do not raise MAX_APY_BPS, remove the daily cap, or add owner mint paths — these are audit-surface decisions for humans.
