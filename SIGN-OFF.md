...
| 1.2 | **Speakeasy bar take** | **$3.12M/day per club** at top tier; **NET ≈ gross, ~3.5–6d payback at every tier** — *now measured (sim P9.12)* | A maxed nightclub earns territory-scale money. The net-EV pass corrected two of my own assumptions: there is **no "pad" upkeep**, and notoriety→Bureau-raids come from the *table/rounds* (patron-driven), **not** the owner's passive collect — so a bar-take-only owner draws **~0 raid tax**. It's still gated (level 15, made man, ~$18.65M to fully build), but once built it's a large, low-risk faucet that pays for itself in under a week. | **WATCH → founder dial.** No §10.4 problem (it's a ledgered faucet), but it's the richest low-risk passive earner in the game. If you want it taxed like other endgame fronts, the dials are (a) a passive-owner notoriety/upkeep drip, or (b) trimming the `incomePerHr` curve. I did **not** retune it (ground rule #1) — flagged for your call with the numbers now in hand. |
