...
`POST /casino/dice` `{amount}` (street craps — the full pass line in one call, pays 1:1, house edge ~1.41%; $100–$250k table, 1 nerve) · `POST /casino/numbers` `{pick, amount}` (the daily Numbers: 0–999, $10–$1k, ONE ticket/street/day, pays 600:1 on the seed-drawn daily number) · `POST /casino/numbers/claim` (settles matured tickets lazily) · `GET /casino` (limits, your tickets, yesterday's number). Every roll server-side + rng-audited; every stake/payout ledgered `casino:*`; 1% of every stake → the street-tax pool (the buyback/yield loop), the rest of the edge burns. **$OMR is never accepted or paid** — the regulatory line. Numbers are founder sign-off levers. Design: `omerta-gambling-den-design.md`.
